...
.\preview.bat examples\starter-panel
```
...
```sh
MODULARUI2_PREVIEW_MINECRAFT_JAR=/path/to/client.jar ./preview.sh examples/starter-panel
```
...
```bat
preview.bat examples\gt5-electrolyzer-direct
```

On Linux or macOS:

```sh
./preview.sh examples/gt5-electrolyzer-direct
```
...
`preview.properties` selects the Java entrypoint and the simulated monitor:
...
Create `src/preview/java/example/MyMachinePreview.java`:
...
public final class MyMachinePreview implements PreviewEntrypoint {
    ...
    public Object createPanel(Context context) {
        return ModularPanel.defaultPanel("my-machine", 176, 166)
            .child(new TextWidget<>("My machine")
                .pos(8, 8));
    }
}
```
...
```bat
preview.bat my-machine-preview
```

The previewer compiles every `.java` file below `src/preview/java` on each run. Edit the layout code and run the same command again to get a new screenshot.
...
```bat
preview.bat project-directory
```

Override the entrypoint:

```bat
preview.bat project-directory fully.qualified.PreviewClass
```

Override the entrypoint and output directory:

```bat
preview.bat project-directory fully.qualified.PreviewClass output/my-iteration
```
